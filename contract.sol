...
contract PaperDistributor is Ownable {
    ...
    uint256 public constant INIT_CLAIM = 100_800_000 * 1e6;
    ...
    function claim(uint128 nonce, bytes calldata signature, address referrer) public {
        require(_usedNonce[nonce] == false, "PAPER: nonce already used");
        require(_claimedUser[_msgSender()] == false, "PAPER: already claimed");

        _claimedUser[_msgSender()] = true;
        require(isValidSignature(nonce, signature), "PAPER: only auth claims");
        
        _usedNonce[nonce] = true;

        uint256 supplyPerAddress = canClaimAmount();
        require(supplyPerAddress >= 1e6, "PAPER: airdrop has ended");
    ...
    function setSigner(address val) public onlyOwner() {
        require(val != address(0), "PAPER: val is the zero address");
        signer = val;
    }
